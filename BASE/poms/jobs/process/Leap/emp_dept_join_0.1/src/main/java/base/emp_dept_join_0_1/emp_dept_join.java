...
package base.emp_dept_join_0_1;
...
/**
 * Job: emp_dept_join Purpose: emp_dept_join<br>
 * Description: emp_dept_join <br>
 * 
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status
 */
public class emp_dept_join implements TalendJob {
	...
	private final String jobName = "emp_dept_join";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					emp_dept_join.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(emp_dept_join.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row5Struct implements routines.system.IPersistableRow<row5Struct> {
		final static byte[] commonByteArrayLock_BASE_emp_dept_join = new byte[0];
		static byte[] commonByteArray_BASE_emp_dept_join = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_BASE_emp_dept_join.length) {
					if (length < 1024 && commonByteArray_BASE_emp_dept_join.length == 0) {
						commonByteArray_BASE_emp_dept_join = new byte[1024];
					} else {
						commonByteArray_BASE_emp_dept_join = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_BASE_emp_dept_join, 0, length);
				strReturn = new String(commonByteArray_BASE_emp_dept_join, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_BASE_emp_dept_join, 0, length);
				strReturn = new String(commonByteArray_BASE_emp_dept_join, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_BASE_emp_dept_join) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_3.addRow(new String[] { "EMPLOYEE_ID", "FIRST_NAME", "LAST_NAME", "EMAIL", "PHONE_NUMBER",
...
///////////////////////    			
				...
				class Util_tJoin_1 {
					...
					public boolean isJoined(row1Struct mainRow) {
						row4HashKey.DEPARTMENT_ID = mainRow.DEPARTMENT_ID;
...
///////////////////////    			
							...
							row5 = new row5Struct();
							row5.EMPLOYEE_ID = row3.EMPLOYEE_ID;
							row5.FIRST_NAME = row3.FIRST_NAME;
							row5.LAST_NAME = row3.LAST_NAME;
							row5.EMAIL = row3.EMAIL;
							row5.PHONE_NUMBER = row3.PHONE_NUMBER;
							row5.HIRE_DATE = row3.HIRE_DATE;
							row5.JOB_ID = row3.JOB_ID;
							row5.SALARY = row3.SALARY;
							row5.COMMISSION_PCT = row3.COMMISSION_PCT;
							row5.MANAGER_ID = row3.MANAGER_ID;
							row5.DEPARTMENT_ID = row3.DEPARTMENT_ID;

							if (util_tJoin_1.isJoined(row3)) {
								row5.DEPARTMENT_NAME = util_tJoin_1.lookupValue.DEPARTMENT_NAME;
							}
...
///////////////////////		
								...
								if (row5.EMPLOYEE_ID != null) { //
									row_tLogRow_3[0] = String.valueOf(row5.EMPLOYEE_ID);

								} //

								if (row5.FIRST_NAME != null) { //
									row_tLogRow_3[1] = String.valueOf(row5.FIRST_NAME);

								} //

								if (row5.LAST_NAME != null) { //
									row_tLogRow_3[2] = String.valueOf(row5.LAST_NAME);

								} //

								if (row5.EMAIL != null) { //
									row_tLogRow_3[3] = String.valueOf(row5.EMAIL);

								} //

								if (row5.PHONE_NUMBER != null) { //
									row_tLogRow_3[4] = String.valueOf(row5.PHONE_NUMBER);

								} //

								if (row5.HIRE_DATE != null) { //
									row_tLogRow_3[5] = String.valueOf(row5.HIRE_DATE);

								} //

								if (row5.JOB_ID != null) { //
									row_tLogRow_3[6] = String.valueOf(row5.JOB_ID);

								} //

								if (row5.SALARY != null) { //
									row_tLogRow_3[7] = String.valueOf(row5.SALARY);

								} //

								if (row5.COMMISSION_PCT != null) { //
									row_tLogRow_3[8] = String.valueOf(row5.COMMISSION_PCT);

								} //

								if (row5.MANAGER_ID != null) { //
									row_tLogRow_3[9] = String.valueOf(row5.MANAGER_ID);

								} //

								if (row5.DEPARTMENT_ID != null) { //
									row_tLogRow_3[10] = String.valueOf(row5.DEPARTMENT_ID);

								} //

								if (row5.DEPARTMENT_NAME != null) { //
									row_tLogRow_3[11] = String.valueOf(row5.DEPARTMENT_NAME);
...
///////////////////////    			
							...
							row4_HashRow.DEPARTMENT_NAME = row4.DEPARTMENT_NAME;
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final emp_dept_join emp_dept_joinClass = new emp_dept_join();

		int exitCode = emp_dept_joinClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = emp_dept_join.class.getClassLoader()
					.getResourceAsStream("base/emp_dept_join_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = emp_dept_join.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : emp_dept_join");
		}
...
/************************************************************************************************
 * 157601 characters generated by Talend Open Studio for Data Integration on the
 * March 20, 2023 at 4:54:00 PM EDT
 ************************************************************************************************/